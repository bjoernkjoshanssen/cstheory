import Myproject2.HydrophobicPolarModel
open Finset

/- Rewritten to fit Mathlib as of June 30, 2024. For some reason `handshake_card₀`
  now has a longer proof.
-/
...
+ card (filter (λ a : α × α ↦ P a.1 a.2 ∧ Q a.2 a.1) univ) := by
  ...
  rw [Nat.add_comm, ← card_union_of_disjoint this]
...
lemma subtype_card_of_eq {b l : ℕ}  (f g : Fin l.succ × Fin b → Prop) (hfg : f = g)
          [DecidablePred f] [DecidablePred g] :
          (Finset.subtype (f) univ).card = (Finset.subtype g univ).card := by
            let W := @Finset.card_equiv
              _ _ univ univ (Equiv.subtypeEquivProp hfg)
              (by simp)
            simp only [Nat.succ_eq_add_one, subtype_univ, card_univ]
            exact W
...
:= by
  ...
  have hf : ∀ a ∈ s, f a ∈ t := by
    intro a;intro;simp; aesop
  have f_inj : ∀ a₁ ∈ s, ∀ a₂ ∈ s, f a₁ = f a₂ → a₁ = a₂ := by
    intro _ _ _ _ hf; apply handshake_map_2_to_2_injective
    exact hfold; exact hf
  simp only [Nat.succ_eq_add_one, Bool.decide_or, Bool.and_eq_true, Bool.or_eq_true,
    decide_eq_true_eq, ge_iff_le]
  have replace_t₁ : (filter (fun ia : Fin l.succ × Fin b ↦ ∃ j,
      (phobic.get ia.1 && phobic.get j && decide (fold.get j = go ia.2 (fold.get ia.1)) &&
      decide ((ia.1.1).succ < ↑j ∨ (j.1).succ < ↑ia.1)) = true) univ).card
      =
      (filter (fun ia : Fin (l+1) × Fin b ↦ ∃ j,
      ((phobic.get ia.1 = true ∧ phobic.get j = true) ∧ fold.get j = go ia.2 (fold.get ia.1)) ∧
      (ia.1.1 + 1 < j.1 ∨ j.1 + 1 < ia.1.1)) univ).card := by
        congr;apply funext;simp

  have replace_t₀: (univ : Finset { x // x ∈ filter (fun ia : Fin l.succ × Fin b ↦
                ∃ j, (phobic.get ia.1 && phobic.get j
                && decide (fold.get j = go ia.2 (fold.get ia.1)) &&
                decide ((ia.1.1).succ < j.1 ∨ (j.1).succ < ia.1.1)) = true) univ }).card
                =
                (filter
                (fun ia : Fin (l+1) × Fin b ↦
                ∃ j, ((phobic.get ia.1 = true ∧ phobic.get j = true)
                ∧ fold.get j = go ia.2 (fold.get ia.1)) ∧
                (ia.1.1 + 1 < j.1 ∨ j.1 + 1 < ia.1.1)) univ).card := by
        rw [card_univ]
        simp -- phew
  ...
  have replace_t₂ : (filter (fun ia : Fin l.succ × Fin b ↦ ∃ j,
      (phobic.get ia.1 && phobic.get j && decide (fold.get j = go ia.2 (fold.get ia.1)) &&
      decide ((ia.1.1).succ < ↑j ∨ (j.1).succ < ↑ia.1)) = true) univ).card = t.card := by
        rw [replace_t₀]
        exact replace_t₁
  have replace_s₁: (filter (fun ij : Fin (l+1) × Fin (l+1) ↦
        ((phobic.get ij.1 = true ∧ phobic.get ij.2 = true) ∧ nearby go (fold.get ij.1) (fold.get ij.2) = true) ∧
        (ij.1.1 + 1 < ij.2.1 ∨ ij.2.1 + 1 < ij.1.1)) univ).card
        =
        (filter (λ ij : (Fin l.succ) × (Fin l.succ) ↦
        phobic.get ij.1 && phobic.get ij.2 && nearby go (fold.get ij.1) (fold.get ij.2)
        && (ij.1.1.succ < ij.2.1 ∨ ij.2.1.succ < ij.1.1)) univ).card := by
          congr;apply funext;simp
  have replace_s₀: (univ : Finset { x // x ∈ filter
    (fun ij : Fin l.succ × Fin l.succ ↦
    (phobic.get ij.1 && phobic.get ij.2 && nearby go (fold.get ij.1) (fold.get ij.2) &&
    decide ((ij.1.1).succ < ↑ij.2 ∨ (ij.2.1).succ < ↑ij.1)) = true)
    univ }).card = (filter (
    λ ij : (Fin l.succ) × (Fin l.succ) ↦
    phobic.get ij.1 && phobic.get ij.2 && nearby go (fold.get ij.1) (fold.get ij.2)
      && (ij.1.1.succ < ij.2.1 ∨ ij.2.1.succ < ij.1.1)
        ...
        simp -- phew
        apply Fintype.card_subtype
  have replace_s₂: (filter
      (fun ij : Fin (l+1) × Fin (l+1) ↦
        ((phobic.get ij.1 = true ∧ phobic.get ij.2 = true) ∧ nearby go (fold.get ij.1) (fold.get ij.2) = true) ∧
          (ij.1.1 + 1 < ij.2.1 ∨ ij.2.1 + 1 < ij.1.1))
      univ).card = s.card := by
    rw [replace_s₀]
    exact replace_s₁

  simp only [Nat.succ_eq_add_one, Bool.decide_or, Bool.and_eq_true, decide_eq_true_eq,
    Bool.or_eq_true] at replace_t₂
  rw [replace_s₂, replace_t₂]
  exact Finset.card_le_card_of_injOn f hf f_inj
