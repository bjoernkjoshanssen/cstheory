...
import Myproject2.Squarefree
import Myproject2.MonoPred
  ...
  | zero =>
    ...
    have : List.ofFn (fun a => ite (M.P (Gap_cons a w (by linarith)).1 ∧ M.Q (Gap_cons a w (by linarith)).1) 1 0)
      ...
      . intro i hi
        ...
        rw [if_neg (still_does_not_hold'' _ _ _)] at hn
        exact hn.symm
        tauto
    rw [this]
    rw [if_neg H]
    ...
    . rw [if_neg H]
      ...
      if M.P (Gap_cons a w hnL).1 then
        ...
        . intro i hi
          ...
          rw [← hn]
          simp
          have Q := @still_does_not_hold b L (n_1+1) M w hnL (Fin.cast (by
...
def getFin {n_1 : ℕ} {i: Fin (Nat.succ n_1)} (hin: ¬i = n_1) : Fin n_1 := by
  have : i.1 < n_1 := by
    ...
    | inr h =>
      exfalso
      have : i = n_1 := by apply Fin.ext; rw [h]; simp only [Fin.natCast_eq_last,
        Fin.val_last]
      exact hin this
  exact ⟨i.1,this⟩
